...
| 位置 | 当前命名 | 问题 | 建议 |
|------|---------|------|------|
| `KMvi` | 全局配置单例 | `K` 前缀作为"Kotlin"的缩写，在此语境中不直观；`KMvi` 读起来像某个第三方工具的品牌名 | `MviConfig` 或 `Mvi.Config` 更清晰 ✅ 已确认，不修改 |
| `HybridConfig` | HYBRID 策略配置 | 名称没有传达出"仅影响 fallback Intent"的含义，让人误以为是所有策略的配置 | `FallbackGroupConfig`、`HybridGroupConfig` 或 `FallbackConfig` ✅ 已确认，不修改 |
...
| `contract()` 扩展函数 | `ViewModel.contract(...)` | 函数名 `contract` 与接口名 `Contract` 同词不同形，在同一代码上下文中出现 `val contract: Contract by contract(...)` 时语义有些重叠 | 可考虑 `mviContract()`（加前缀区分） ✅ 已确认，不修改 |
| `IntentHandlerDelegate` | 内部类 | `Delegate` 后缀是实现细节，不够描述性 | 已是 `internal`，可接受；若重命名可考虑 `IntentHandlerDispatcher` ✅ 已确认，不修改 |
...
- ~~`ReactiveContractLazy` 已有注释说明"非线程安全"，但未说明*为何*不做线程安全（ViewModel 主线程假设），补充该说明可防止未来误用。~~ ✅ 已修复（见 §4.7）
- `MviCollects.kt` 中的 `launchWithLifecycle`：文档说"All collections are launched on the main thread"，但实际上 `context` 参数中可以携带 `Dispatcher`，该断言不完全准确，建议修正为"默认在主线程，可通过 `context` 参数覆盖"。 ✅ 已确认，不修改
...
| `doOnClick` / `doOnLongClick` / `doOnCheckedChange` / `doOnAfterTextChanged` | 无测试（需 Android 环境） ✅ 已确认，不补测 |
| `StateCollector` / `EventCollector` | 无测试（需要 `LifecycleOwner`，可用 Robolectric） ✅ 已确认，不补测 |
| `launchCollect` / `launchWithLifecycle` | 无测试 ✅ 已确认，不补测 |
| `Mvi.Snapshot` 方法（`updateState`/`withEvent`/`updateWith`） | `MviTest` 存在，但需确认 ✅ 已确认，不补测 |
| `groupHandle` 的 stale channel 路径 | 未见专项测试 ✅ 已确认，不补测 |
...
| 命名 | 低 | `KMvi` 名称不直观 ✅ 已确认，不修改 |
| 命名 | 低 | `HybridConfig` 未体现"仅针对 fallback"的语义 ✅ 已确认，不修改 |
...
| Kotlin 写法 | 低 | `Snapshot` 方法内多余的 `this.`，`doOnClick` 中多余的 `this.block()` ✅ 已确认，不修改 |
...
| 文档 | 低 | 文件中 Date 注脚静态过时 ✅ 已确认，不修改 |
| 文档 | 低 | `launchWithLifecycle` 文档"主线程"说法不完全准确 ✅ 已确认，不修改 |
| 架构 | 低 | ~~`sealed interface Mvi` 的 sealed 限制没有实际意义~~ ✅ 已修复：改为 `object Mvi` |
| 架构 | 低 | ~~`Snapshot.of()` API 表面多余~~ ✅ 已修复 |
| 测试 | 中 | ~~`debounceLeading`~~（已修复）、UI 事件转换函数、`StateCollector`/`EventCollector` 缺少测试 ✅ 已确认，不补测 |
